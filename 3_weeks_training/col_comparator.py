...
import pickle 
X = pd.read_csv('/home/victor/Documentos/GitHub/nasdaq/3_weeks_training/X_best_columns_3W_43.csv')
y = pd.read_csv('y.csv')
...
for i in range(0,100):
    if i % 2 == 0:
        cols = MyUtils.get_nth_combination(X.columns,47,i*1000)
    else: 
        cols = MyUtils.get_nth_combination(X.columns,47,(1000000-i))
    X_random = X.loc[:,cols]
...
with open('/home/victor/Documentos/GitHub/nasdaq/3_weeks_training/3W_43_random.pickle', 'wb') as handle:
    pickle.dump(score_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('/home/victor/Documentos/GitHub/nasdaq/3_weeks_training/3W_43_score.pickle', 'wb') as handle:
