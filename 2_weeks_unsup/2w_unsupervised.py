...
col_list_2W_43 = MyUtils.get_all_smallest(r_dict_2W_14, 74)

score_list_2W_14 = []
my_weights =[]
for i in range(0,183):
  limit = 6250+125*i+374
  if limit < 18750:
    reg=LinearRegression().fit(X[col_list_2W_43][(6250+i*125):(6250+249+i*125)], y[(6250+i*125):(6250+249+i*125)])
    #print(reg.score(X[col_list][(2787+125*i+250):(5574+125*i+374)], y[(5574+125*i+250):(5574+125*i+374)]))
    my_weights = MyUtils.solver(X[col_list_2W_43][(6250+i*125):(6250+249+i*125)], y[(6250+i*125):(6250+249+i*125)])
    reg.coef_=my_weights.ravel()
    reg.intercept_=0    
    score_list_2W_14.append(reg.score(X[col_list_2W_43][(6250+125*i+250):(6250+125*i+374)], y[(6250+125*i+250):(6250+125*i+374)]))
  else:
    break
...
#print(6250+125*i+374)
avg_2w_14_col=np.mean(score_list_2W_14)
print(reg.coef_)

with open('/home/victor/Documentos/GitHub/nasdaq/2_weeks_unsup/2W_74_random.pickle', 'wb') as handle:
