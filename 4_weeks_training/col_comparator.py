...
import pickle 
X = pd.read_csv('/home/victor/Documentos/GitHub/nasdaq/4_weeks_training/X_best_columns_4W_74.csv')
y = pd.read_csv('y.csv')
...
for i in range(0,100):
    ...
    for j in range(0,iters):
        temp = X_random
        #X = X.drop(X.columns[-1], axis=1)
        start= MyUtils.WEEKLY_SAMPLES*j*4
        finish = (MyUtils.WEEKLY_SAMPLES*j*4+MyUtils.WEEKLY_SAMPLES)
        test_start = (MyUtils.WEEKLY_SAMPLES*j*4+MyUtils.WEEKLY_SAMPLES+1)
        test_finish = (MyUtils.WEEKLY_SAMPLES*j*4+MyUtils.WEEKLY_SAMPLES+1+MyUtils.WEEKLY_SAMPLES)
...
with open('/home/victor/Documentos/GitHub/nasdaq/4_weeks_training/4W_74_random.pickle', 'wb') as handle:
    pickle.dump(score_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('/home/victor/Documentos/GitHub/nasdaq/4_weeks_training/4W_74_score.pickle', 'wb') as handle:
